...
public class DoorSystem1 : MonoBehaviour
{
    ...
    private void OpenDoorCondition()
    {
        for (int i = 0; i < Doors; i++)
        {
            if (_button[0])
            {
                _doors[0].gameObject.active = false;
                _doors[1].gameObject.active = true;
            }
            else
            {
                _doors[0].gameObject.active = true;
                _doors[1].gameObject.active = false;
            }
        }
    }
